...
$(document).ready(function () {
  ...
  $("#submitBrandForm")
    ...
    .bind("submit", function () {
      ...
      if (price == "") {
        $("#price").after(
          '<p class="text-danger">Price/Gram field is required</p>'
        );
      ...
      if (brandName && price && brandStatus) {
        var form = $(this);
        // button loading
...
function addCategories(brandId = null, brand_type) {
  if (brandId) {
    $.ajax({
      ...
      dataType: "json",
      success: function (response) {
        location.href = "categories.php?brandId=" + brandId + "&brandType="+ brand_type;
      },
      error: function (url) {
        alert(error);
      },
...
function editBrands(brandId = null) {
  if (brandId) {
    ...
    $.ajax({
      ...
      dataType: "json",
      success: function (response) {
        if(response.brand_type == 2){
          document.getElementById("editQuantityControl").style.display = "none";
        }
        // modal loading
